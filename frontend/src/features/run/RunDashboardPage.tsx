...
export function RunDashboardPage() {
  ...
  const displayedStage = currentRun.stages.find(s => s.id === currentStageId) || activeStage
        ...
        <section className="workspace" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div className="run-breadcrumb" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span className="demo-chip" style={{ border: '1px solid rgba(110,231,183,.2)', color: 'var(--success)', background: 'rgba(110,231,183,.08)' }}>
                Demo Sandbox
              </span>
            </div>
            
            <div className="demo-controls" style={{ display: 'flex', gap: '8px', background: 'var(--surface-muted)', padding: '4px', borderRadius: '8px', border: '1px solid var(--border)' }}>
              <button onClick={() => setDemoState('Running')} className={demoState === 'Running' ? 'demo-active' : ''} style={{ fontSize: '10px', padding: '4px 8px', borderRadius: '4px', color: demoState === 'Running' ? 'var(--bg)' : 'var(--text-muted)', background: demoState === 'Running' ? 'var(--signal-bright)' : 'transparent', fontWeight: 700 }}>Running</button>
              <button onClick={() => setDemoState('Verified')} className={demoState === 'Verified' ? 'demo-active' : ''} style={{ fontSize: '10px', padding: '4px 8px', borderRadius: '4px', color: demoState === 'Verified' ? 'var(--bg)' : 'var(--text-muted)', background: demoState === 'Verified' ? 'var(--success)' : 'transparent', fontWeight: 700 }}>Verified</button>
              <button onClick={() => setDemoState('Rolled back')} className={demoState === 'Rolled back' ? 'demo-active' : ''} style={{ fontSize: '10px', padding: '4px 8px', borderRadius: '4px', color: demoState === 'Rolled back' ? 'var(--bg)' : 'var(--text-muted)', background: demoState === 'Rolled back' ? 'var(--danger)' : 'transparent', fontWeight: 700 }}>Rolled back</button>
            </div>
          </div>
          ...
          <header style={{ borderBottom: '1px solid var(--border)', paddingBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '13px', color: 'var(--text-secondary)' }}>
              <span>Target App: <strong style={{ color: 'var(--text)' }}>{currentRun.targetName}</strong></span>
              <span style={{ color: 'var(--border)' }}>|</span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                <ExternalLink size={12} /> URL: <code>{currentRun.targetUrl}</code>
              </span>
            </div>
          </header>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {displayedStage.id === 'repair' && (
                  <RepairVisualizer data={
                    displayedStage.status === 'queued' ? null : mockSnapshot.repair_snapshot
                  } />
                )}
                {displayedStage.id === 'verifier' && (
                  <VerifierVisualizer data={
                    displayedStage.status === 'queued' ? null : mockSnapshot.verification_snapshot
                  } />
                )}
              </div>
            </article>
              ...
              <article className="panel-card" style={{ padding: '16px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                ...
                <ol className="event-list" style={{ margin: 0, overflowY: 'auto', maxHeight: '240px', flex: 1 }}>
                  {currentRun.stages.map((s, i) => (
                    <li key={s.id} style={{ fontSize: '11px', display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 0' }}>
                      <span className={`event-dot ${s.status === 'running' ? 'running pulse' : s.status === 'complete' ? 'complete' : s.status === 'failed' ? 'failed' : 'queued'}`} />
                      <span style={{ flex: 1, color: 'var(--text-secondary)', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
