...
const STAGE_DEFS = [
  ...
  },
  {
    id: "mapper" as const,
    name: "Source Mapper",
    snapshotKey: "source_snapshot",
    metricKey: "Source Mapper",
    snapshot: "SourceSnapshot",
  },
  {
    id: "analyzer" as const,
    name: "Analyzer",
    snapshotKey: "analysis_snapshot",
    metricKey: "Analyzer",
  ...
  },
  {
    ...
    name: "Verifier",
    snapshotKey: "verification_snapshot",
    metricKey: "Verifier",
...
function deriveStages(
  ...
  const stages: PipelineStage[] = STAGE_DEFS.map((def, i) => {
    ...
    if (hasSnapshot) {
      ...
      firstMissing = i;
      const failedEvent = snap.events.find(
        (e) => e.stage === def.metricKey && e.status === "failed",
      );
      const skippedEvent = snap.events.find(
        (e) => e.stage === def.metricKey && e.status === "skipped",
      );
      if (failedEvent) {
        status = "failed";
...
function extractSummary(stageId: string, snap: PipelineSnapshot): string {
  ...
  if (stageId === "explorer" && s.explorer_snapshot) {
    ...
    return meta?.success
      ? "Runtime evidence captured successfully."
      : "Explorer encountered an error.";
  }
...
function extractFacts(stageId: string, snap: PipelineSnapshot): string[] {
  ...
  if (stageId === "mapper" && s.source_snapshot) {
    ...
    return (files ?? [])
      ...
      .map(
        (f: any) =>
          `${f.file_path} (${(f.components as unknown[])?.length ?? 0} components)`,
      );
  }
  if (stageId === "analyzer" && s.analysis_snapshot) {
    const as_ = s.analysis_snapshot;
    ...
    return (hyps ?? []).map(
      (h: any) => `${h.hypothesis} [${h.plausibility_score}]`,
    );
  }
  if (stageId === "repair" && s.repair_snapshot) {
    const rs = s.repair_snapshot;
...
export function LiveRunDashboardPage({ runId }: Props) {
  ...
  const displayedStage =
    stages.find((s) => s.id === currentStageId) || activeStage;
  ...
  const elapsed =
    data?.snapshot?.total_runtime_seconds != null
      ...
      <AppShell showBack>
        ...
        >
          <div style={{ textAlign: "center", maxWidth: "420px" }}>
            ...
            <h2 style={{ margin: "12px 0 8px", fontSize: "20px" }}>
              Connection failed
            </h2>
            <p
              style={{
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              <span
                className="demo-chip"
                style={{
                  border: "1px solid rgba(139,140,255,.2)",
            ...
            <div className="run-status-cluster" style={{ gap: "16px" }}>
              ...
              <span style={{ color: "var(--text-secondary)" }}>
                <Clock3
                  size={13}
                  style={{ marginRight: "4px", inlineSize: "auto" }}
                />{" "}
                Duration: {elapsed}
              </span>
            </div>
            ...
            >
              ...
              >
                {displayedStage.id === "explorer" && (
                  <ExplorerVisualizer
                    data={data?.snapshot?.explorer_snapshot}
                  />
                )}
                {displayedStage.id === "mapper" && (
                  <SourceMapperVisualizer
                    data={data?.snapshot?.source_snapshot}
                  />
                )}
                {displayedStage.id === "analyzer" && (
                  <AnalyzerVisualizer
                    data={data?.snapshot?.analysis_snapshot}
                  />
                )}
                ...
                {displayedStage.id === "verifier" && (
                  <VerifierVisualizer
                    data={data?.snapshot?.verification_snapshot}
                  />
                )}
              </div>
            </article>
                  ...
                  {stages.map((st) => {
                    ...
                    return (
                      <button
                        ...
                        aria-label={`View ${st.name} stage details`}
                        style={{
                          display: "flex",
                          justifyContent: "space-between",
                          alignItems: "center",
              ...
              >
                ...
                >
                  {(data?.snapshot?.execution_history ?? []).map((entry, i) => (
                    ...
                    >
                      <span
                        className={`event-dot ${entry.toLowerCase().includes("completed") ? "complete" : entry.toLowerCase().includes("started") ? "running" : entry.toLowerCase().includes("failed") || entry.toLowerCase().includes("skipped") ? "failed" : "queued"}`}
                      />
                      <span
                        style={{
                          flex: 1,
                  ...
                  {!data?.snapshot?.execution_history?.length && (
                    ...
                    >
                      <span className="event-dot queued" />
